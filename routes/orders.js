...
// @access  Private (Buyer only)
router.get("/", auth, authorize("buyer", "admin"), asyncHandler(async (req, res) => {
  const options = {
...
router.get("/all", auth, authorize("admin"), asyncHandler(async (req, res) => {
  const options = {
    page: parseInt(req.query.page) || 1,
    limit: parseInt(req.query.limit) || 10,
    status: req.query.status,
...
// @access  Private (Seller - own products, Admin - all orders)
router.get("/stats/overview", auth, authorize("seller", "admin"), asyncHandler(async (req, res) => {
